...
namespace XmlRpcWordpress
{
    ...
    {
        private readonly IWordpressBlog _proxy;
        ...
        public WordpressBlog(string username, string password, string url)
        {
            BlogId = BlogId <= 0 ? 1 : BlogId;
            Username = username;
            Password = password;

            _proxy = (IWordpressBlog)XmlRpcProxyGen.Create(typeof(IWordpressBlog));

            url += !url.EndsWith("\x002F") ? "\x002F" : "";
            _proxy.Url = url + @"xmlrpc.php";
        }

        //posts ----------------------------------------------------------------------------------------------------

        public string CreatePost(Content content, bool publish)
        {
            return _proxy.CreatePost(1, Username, Password, content, publish);
        }

        public bool EditPost(string postId, Content content, bool publish)
        {
            return _proxy.EditPost(postId, Username, Password, content, publish);
        }

        public object DeletePost(string postId)
        {
            return _proxy.DeletePost(string.Empty, postId, Username, Password, true);
        }

        public BlogPost[] GetRecentPosts(int numberOfPosts)
        {
            return _proxy.GetRecentPosts(BlogId, Username, Password, numberOfPosts);
        }

        public BlogPost GetPost(int postId)
        {
            return _proxy.GetPost(postId, Username, Password);
        }

        //categories, Keywords  -------------------------------------------------------------------------------------
        public Category[] GetCategories()
        {
            return _proxy.GetCategories(BlogId, Username, Password);
        }

        public Tag[] GetTags()
        {
            return _proxy.GetTags(BlogId, Username, Password);
        }
        ...
        {
            return _proxy.GetUserBlogs(String.Empty, Username, Password);
        }

        public object GetUserInfo()
        {
            return _proxy.GetUserInfo(String.Empty, Username, Password);
        }

        public object GetOptions(string[] options = null)
        {
            return _proxy.GetOptions(BlogId, Username, Password, options ?? new string[0]);
        }

        public object GetSupportedMethods()
        {
            return _proxy.GetSupportedMethods();
        }



        //comments --------------------------------------------------------------------------------------------------
        public Comment[] GetComments(CommentFilter commentMeta)
        {
            return _proxy.GetComments(BlogId, Username, Password, commentMeta);
        }
        public CommentCount GetCommentCount(string postId)
        {
            return _proxy.GetCommentCount(BlogId, Username, Password, postId);
        }

        public bool DeleteComment(int commentId)
        {
            return _proxy.DeleteComment(BlogId, Username, Password, commentId);
        }

        public object GetCommentStatusList()
        {
            return _proxy.GetCommentStatusList(BlogId, Username, Password);
        }
        ...
        public MediaObjectResult SaveMedia(string mediaFilePath, bool overwrite)
        {
            var mediaObject = new MediaObject();
            byte[] file;
            var info = new FileInfo(mediaFilePath);

            using (var fs = new FileStream(mediaFilePath, FileMode.Open, FileAccess.Read))
            {
                file = new byte[fs.Length];
                fs.Read(file, 0, (int)fs.Length);
            }

            mediaObject.name = info.Name;
            mediaObject.type = GetMimeType(info.Extension);
            mediaObject.bits = file;
            mediaObject.overwrite = overwrite;

            return _proxy.NewMediaObject(BlogId, Username, Password, mediaObject);
        }
